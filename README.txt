...
utils.py: utils script used in evaluation files for trial set. This script also contains several elaborate print statements, created for verification purposes throughout the development stages of the code.
...
utils_test.py: utils script used in evaluation files for test set. Same as utils.py, but used for test set evaluation, plus code for post-evaluation experiments and analyses. This script only contains print statements where required.
...
10. post-eval_experiments_SG_robertalarge_test.ipynb. This file corresponds to post-evaluation experiment and results presented in section 5.5 of the thesis. 
11. post_eval_experiments_SR_Hyper-Hypo_test.ipynb. This file corresponds to post-evaluation experiments and results presented in section 5.5 of the thesis. 
12. post-eval_analysis_SR_CEFR_test.ipynb. This file corresponds to a post-evaluation analysis presented in section 6.4 of the thesis.


- the following evaluation scripts to calculate the 10 metrics for the Shared Task:
...
tsar_eval.py: pre-supplied script (with small update to correct an error, see above file 'tsar_eval-orig_noEncoding.py') to calculate the 10 evaluation metrics. This script was called from the command line to evaluate the different files in the predictions folder, and the results are recorded in the evaluations and post-evaluations files mentioned above. 
