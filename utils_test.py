...
def substitute_selection_phase_2_option_2(data, substitutes_df, lm_tokenizer, fill_mask, model_name_str, nlp, levels=[1, 2]):
    ...
    # export the dataframe to tsv for evaluation
    substitutes_df.to_csv(f"./predictions/test/SS_phase2_option2_SharedHyper{'_'.join(map(str, levels))}_{model_name_str}.tsv", sep="\t", index=False, header=False)
    print(f"SS_phase2_option2_SharedHyper{'_'.join(map(str, levels))}_{model_name_str} exported to csv in path './predictions/test/SS_phase2_option2_SharedHyper{'_'.join(map(str, levels))}_{model_name_str}.tsv'\n")
...
def substitute_ranking_option1_hyper(prediction, levels=[1, 2]):
    ...
    # export the dataframe to tsv for evaluation
    pred_df.to_csv(f"./predictions/test/{predict_abbrev}_SR_option1_Hyper{'_'.join(map(str, levels))}-Hypo_robertabase.tsv", sep="\t", index=False, header=False)
    print(f"{predict_abbrev}_SR_option1_Hyper{'_'.join(map(str, levels))}-Hypo_robertabase exported to path './predictions/test/{predict_abbrev}_SR_option1_Hyper{'_'.join(map(str, levels))}-Hypo_robertabase.tsv'\n")
...
def posteval_analysis_cefr_weighted_stats_complexword_substitutes(weighted_dataset, prediction):
    ...
    for index, row in pred_df.iterrows():
        ...
        # get the pos tag of the complex word and its lemma
